# Sourced by setup-rules.sh and setup-agents.sh. Do not execute directly.
# Shared constants, config loading, state, scope, cleanup, get_*, generation helpers.
#
...
readonly CLAUDE_SUBAGENTS_FILE="$SCRIPT_DIR/claude-subagents.json"
readonly CLAUDE_CUSTOM_AGENTS_FILE="$PROJECT_ROOT/.ai-iap-custom/claude-agents.json"
...
# Set after select_scope(): where to write generated files and state
# project -> PROJECT_ROOT; global -> $HOME
OUTPUT_ROOT="$PROJECT_ROOT"
STATE_FILE="$PROJECT_ROOT/.ai-iap-state.json"
SCOPE="project"
# Set after select_setup_type(): rules | agents
SETUP_TYPE="rules"
...
load_previous_state() {
    ...
    while IFS= read -r id; do
        [[ -n "$id" ]] && PREVIOUS_SELECTED_CLAUDE_SUBAGENTS+=("$id")
    done < <(jq -r '.selectedClaudeSubagents[]? // empty' "$STATE_FILE")

    PREVIOUS_SELECTED_CLAUDE_CUSTOM_DEFINED_AGENTS=()
    while IFS= read -r line; do
        [[ -n "$line" ]] && PREVIOUS_SELECTED_CLAUDE_CUSTOM_DEFINED_AGENTS+=("$line")
    done < <(jq -c '.selectedCustomAgents[]? // empty' "$STATE_FILE" 2>/dev/null)

    PREVIOUS_SCOPE="$(jq -r '.scope // "project"' "$STATE_FILE")"
    PREVIOUS_SETUP_TYPE="$(jq -r '.setupType // "both"' "$STATE_FILE")"
...
print_previous_state_summary() {
    echo ""
    local state_display="$STATE_FILE"
    [[ "$STATE_FILE" == "$PROJECT_ROOT"* ]] && state_display="${STATE_FILE#"$PROJECT_ROOT/"}"
    printf '%b\n' "${BOLD}Previous setup detected${NC} ($state_display, scope: ${PREVIOUS_SCOPE:-project}, type: ${PREVIOUS_SETUP_TYPE:-both})"
...
# Ask where to apply config: this project or global (user home). Sets OUTPUT_ROOT, STATE_FILE, SCOPE.
select_scope() {
    echo "Where should the configuration be applied?"
    echo "  1. This project  - write to current directory (e.g. .claude/, .cursor/); use for version-controlled, per-repo setup"
    echo "  2. Global (user) - write to your home directory (~/.claude/, ~/.cursor/); use for personal defaults across all projects"
    echo ""
    read -rp "Enter choice (1 or 2) [1]: " choice
    choice="${choice:-1}"
    if [[ "$choice" == "2" ]]; then
        OUTPUT_ROOT="${HOME:-$HOME}"
        STATE_FILE="$OUTPUT_ROOT/.ai-iap-state.json"
        SCOPE="global"
        print_info "Scope: global → $OUTPUT_ROOT"
    else
        OUTPUT_ROOT="$PROJECT_ROOT"
        STATE_FILE="$OUTPUT_ROOT/.ai-iap-state.json"
        SCOPE="project"
        print_info "Scope: project → $OUTPUT_ROOT"
    fi
    echo ""
}

# Ask what to set up: rules only or agents only. Sets SETUP_TYPE. Skip if SETUP_TYPE already set (e.g. --rules-only).
# Use ./.ai-iap/setup-rules.sh or ./.ai-iap/setup-agents.sh to run one or the other directly.
select_setup_type() {
    [[ -n "${SETUP_TYPE_FROM_ARG:-}" ]] && SETUP_TYPE="$SETUP_TYPE_FROM_ARG" && return 0
    echo "What do you want to set up?"
    echo "  1. Rules only  - languages, frameworks, tools (Cursor, Claude rules, Copilot, etc.)"
...
cleanup_managed_cursor_rules() {
    local root="$OUTPUT_ROOT/.cursor/rules"
    [[ ! -d "$root" ]] && return 0
...
cleanup_managed_claude_rules() {
    local root="$OUTPUT_ROOT/.claude/rules"
    [[ ! -d "$root" ]] && return 0
...
cleanup_managed_claude_agents() {
    local root="$OUTPUT_ROOT/.claude/agents"
    [[ ! -d "$root" ]] && return 0
...
cleanup_tool_outputs() {
    ...
    case "$tool" in
        ...
        claude)
            ...
            ;;
        github-copilot)
            cleanup_generated_file_if_managed "$OUTPUT_ROOT/.github/copilot-instructions.md"
            ;;
        windsurf)
            cleanup_generated_file_if_managed "$OUTPUT_ROOT/.windsurfrules"
            ;;
        aider)
            cleanup_generated_file_if_managed "$OUTPUT_ROOT/CONVENTIONS.md"
            ;;
        google-ai-studio)
        ...
        amazon-q)
            ...
            ;;
        tabnine)
            cleanup_generated_file_if_managed "$OUTPUT_ROOT/TABNINE.md"
            ;;
        cody)
            cleanup_generated_file_if_managed "$OUTPUT_ROOT/.cody/instructions.md"
            ;;
        continue)
            cleanup_generated_file_if_managed "$OUTPUT_ROOT/.continue/instructions.md"
            ;;
    esac
}
...
# Returns ruleBindings JSON for one agent. Presets 1-6 or custom (7) with language/framework pick.
# Output: compact JSON object e.g. {"general":[],"swift":["ios","swiftui"]}
select_tech_stack_for_agent() {
    echo ""
    echo "Tech stack presets (rules to inject for this agent):"
    echo "  1) iOS              - Swift, iOS, SwiftUI"
    echo "  2) Vue.js           - TypeScript, Vue"
    echo "  3) PHP Laravel      - PHP, Laravel"
    echo "  4) SEO              - general only"
    echo "  5) UI/UX            - general, HTML, CSS, Tailwind"
    echo "  6) SEO & Linguistic - general only (SEO + language/copy)"
    echo "  7) Custom           - pick languages and frameworks yourself"
    echo ""
    read -rp "Enter choice (1-7) [1]: " choice
    choice="${choice:-1}"

    case "$choice" in
        1) echo '{"general":[],"swift":["ios","swiftui"]}' ;;
        2) echo '{"general":[],"typescript":["vue"]}' ;;
        3) echo '{"general":[],"php":["laravel"]}' ;;
        4|6) echo '{"general":[]}' ;;
        5) echo '{"general":[],"html":[],"css":["tailwind"]}' ;;
        7)
            local bindings='{}'
            local lang_keys=()
            while IFS= read -r k; do lang_keys+=("$k"); done < <(get_languages)
            echo "Select languages (rules from these will be injected). Example: 1 2 5 for general, swift, php."
            echo ""
            for ((i=0; i<${#lang_keys[@]}; i++)); do
                local lang_name
                lang_name=$(get_language_name "${lang_keys[$i]}" 2>/dev/null)
                echo "  $((i+1)). ${lang_keys[$i]}${lang_name:+ ($lang_name)}"
            done
            echo ""
            read -rp "Enter language numbers (space-separated): " lang_nums
            local lang
            for num in $lang_nums; do
                local idx=$((num - 1))
                [[ $idx -lt 0 || $idx -ge ${#lang_keys[@]} ]] && continue
                lang="${lang_keys[$idx]}"
                local fw_keys=()
                while IFS= read -r f; do [[ -n "$f" ]] && fw_keys+=("$f"); done < <(get_language_frameworks "$lang" 2>/dev/null)
                if [[ ${#fw_keys[@]} -eq 0 ]]; then
                    bindings=$(jq -c --arg lang "$lang" '. + {($lang): []}' <<<"$bindings")
                else
                    echo "  Frameworks for $lang:"
                    for ((i=0; i<${#fw_keys[@]}; i++)); do
                        echo "    $((i+1)). ${fw_keys[$i]}"
                    done
                    read -rp "  Enter framework numbers (space-separated, or Enter for none): " fw_nums
                    local fw_list="[]"
                    if [[ -n "$fw_nums" ]]; then
                        local fws=()
                        for n in $fw_nums; do
                            local j=$((n - 1))
                            [[ $j -ge 0 && $j -lt ${#fw_keys[@]} ]] && fws+=("${fw_keys[$j]}")
                        done
                        fw_list=$(printf '%s\n' "${fws[@]}" | jq -R . | jq -s .)
                    fi
                    bindings=$(jq -c --arg lang "$lang" --argjson fws "$fw_list" '. + {($lang): $fws}' <<<"$bindings")
                fi
            done
            [[ "$bindings" == "{}" ]] && bindings='{"general":[]}'
            echo "$bindings"
            ;;
        *) echo '{"general":[]}' ;;
    esac
...
save_state() {
    local tools_json langs_json docs_json fw_json structs_json procs_json subagents_json custom_agents_json
    ...
    subagents_json=$(json_array_from_items "${SELECTED_CLAUDE_SUBAGENTS[@]:-}")
    custom_agents_json=$(printf '%s\n' "${SELECTED_CLAUDE_CUSTOM_DEFINED_AGENTS[@]:-}" | jq -s 'map(fromjson)' 2>/dev/null || echo "[]")

    jq -n \
        --arg version "$VERSION" \
        --arg scope "${SCOPE:-project}" \
        --arg setupType "${SETUP_TYPE:-both}" \
...
# Build prompt body from ruleBindings (JSON object: lang -> array of framework keys).
# Used for role-based and custom agents so they get project rules injected.
# Optional second arg: persona_specialization (software|seo|ui-ux|testing|devops). When set, for general "persona"
# we inject persona-core + persona-specialist-{persona_specialization} instead of full persona.md (one agent, one specialisation).
inject_rules_for_bindings() {
    local bindings_json="$1"
    local persona_specialization="${2:-}"
    [[ -z "$bindings_json" || "$bindings_json" == "null" ]] && return 0

    local lang
    while IFS= read -r lang; do
        [[ -z "$lang" || "$lang" == "null" ]] && continue
        local fw_list
        fw_list=$(jq -r --arg lang "$lang" '.[$lang] // [] | .[]?' <<<"$bindings_json" 2>/dev/null)
        # Language core files
        while IFS= read -r file; do
            ...
            # For agents: inject split persona (core + specialist) when persona_specialization is set
            if [[ "$lang" == "general" && "$file" == "persona" && -n "$persona_specialization" && "$persona_specialization" != "generic" ]]; then
                if [[ "$persona_specialization" == "software" || "$persona_specialization" == "seo" || "$persona_specialization" == "ui-ux" || "$persona_specialization" == "testing" || "$persona_specialization" == "devops" ]]; then
                    read_instruction_file "$lang" "persona-core" 2>/dev/null || true
                    echo ""
                    read_instruction_file "$lang" "persona-specialist-${persona_specialization}" 2>/dev/null || true
                else
                    read_instruction_file "$lang" "$file" 2>/dev/null || true
                fi
            else
                read_instruction_file "$lang" "$file" 2>/dev/null || true
            fi
            echo ""
        done < <(get_language_files "$lang" 2>/dev/null)

        # Framework files for this language
        local fw
        for fw in $fw_list; do
            [[ -z "$fw" || "$fw" == "null" ]] && continue
            local fw_file
            fw_file=$(get_framework_file "$lang" "$fw" 2>/dev/null)
            [[ -n "$fw_file" && "$fw_file" != "null" ]] && read_instruction_file "$lang" "$fw_file" "true" 2>/dev/null || true
            echo ""
        done
    done < <(jq -r 'keys[]?' <<<"$bindings_json" 2>/dev/null)
}
...
get_optional_rule_keys() {
    jq -r ".languages[\"$1\"].optionalRules // {} | keys_unsorted[]" "$WORKING_CONFIG" 2>/dev/null || true
}

get_optional_rule_file() {
    jq -r ".languages[\"$1\"].optionalRules[\"$2\"].file" "$WORKING_CONFIG"
}

get_optional_rule_toggle() {
    jq -r ".languages[\"$1\"].optionalRules[\"$2\"].toggle" "$WORKING_CONFIG"
}
...
select_tools_simple() {
    ...
    while IFS= read -r key; do
        # Global scope: only Cursor and Claude support user-level config
        [[ "$SCOPE" == "global" ]] && [[ "$key" != "cursor" && "$key" != "claude" ]] && continue
        tool_keys+=("$key")
        tools+=("$(get_tool_name "$key")")
    done < <(get_tools)
    
    if [[ "$SCOPE" == "global" ]]; then
        echo "Select AI tools to configure (global scope: only Cursor and Claude):"
    else
...
# Claude Code subagents (array of template ids) and custom-defined (array of JSON strings per agent)
declare -a SELECTED_CLAUDE_SUBAGENTS=()
declare -a SELECTED_CLAUDE_CUSTOM_DEFINED_AGENTS=()
...
# Agents are always user-defined: name, description, tech stack per agent. No presets.
select_claude_subagents() {
    SELECTED_CLAUDE_CUSTOM_DEFINED_AGENTS=()

    echo ""
    echo "Claude Code agents: you define each agent (name, description, tech stack)."
    echo "Example: 4 agents - one for iOS, one for Vue.js, one for SEO and Linguistic, one for PHP with Laravel."
    echo "Files are written to .claude/agents/ (or global scope)."
    echo ""
    read -rp "Set up Claude Code agents? (y/N): " do_subagents
    if [[ ! "$do_subagents" =~ ^[Yy]$ ]]; then
        return 0
    fi

    echo ""
    read -rp "How many agents do you want to define? (e.g. 4): " num_custom
    num_custom=$((num_custom + 0))
    if [[ $num_custom -gt 0 ]]; then
        local i=1 name desc bindings_json persona_spec agent_json
        while [[ $i -le $num_custom ]]; do
            echo ""
            echo "--- Agent $i of $num_custom ---"
            read -rp "Agent name (slug, e.g. ios-developer): " name
            [[ -z "$name" ]] && name="agent-$i"
            name=$(echo "$name" | tr '[:upper:]' '[:lower:]' | tr -cs 'a-z0-9' '-')
            read -rp "Description (when Claude should use this agent): " desc
            [[ -z "$desc" ]] && desc="Custom agent $i."
            bindings_json=$(select_tech_stack_for_agent)
            echo ""
            echo "Persona specialisation (one agent = one focus):"
            echo "  1) Software/Developer - iOS, PHP, Vue, backend, frontend (default)"
            echo "  2) SEO                - discoverability, meta, structured data"
            echo "  3) UI/UX              - design system, components, accessibility"
            echo "  4) Testing/QA         - test strategy, unit/integration/e2e, automation"
            echo "  5) DevOps/SRE         - CI/CD, infra, deployment, observability"
            echo "  6) Generic            - full adaptive persona (asks about user role)"
            read -rp "Choice (1-6) [1]: " persona_choice
            persona_choice="${persona_choice:-1}"
            case "$persona_choice" in
                2) persona_spec="seo" ;;
                3) persona_spec="ui-ux" ;;
                4) persona_spec="testing" ;;
                5) persona_spec="devops" ;;
                6) persona_spec="generic" ;;
                *) persona_spec="software" ;;
            esac
            agent_json=$(jq -n \
                --arg name "$name" \
                --arg desc "$desc" \
                --argjson ruleBindings "$bindings_json" \
                --arg personaSpecialization "$persona_spec" \
                --arg tools "Read, Glob, Grep, Write, Edit, Bash" \
                --arg model "sonnet" \
                '{name: $name, description: $desc, ruleBindings: $ruleBindings, personaSpecialization: $personaSpecialization, tools: $tools, model: $model}')
            SELECTED_CLAUDE_CUSTOM_DEFINED_AGENTS+=("$agent_json")
            ((i++)) || true
        done
    fi
}
...
generate_cursor() {
    local output_dir="$OUTPUT_ROOT/.cursor/rules"
...
generate_claude() {
    ...
    local output_dir="$OUTPUT_ROOT/.claude/rules"
    ...
    for lang in "${SELECTED_LANGUAGES[@]}"; do
        ...
        for opt_key in $(get_optional_rule_keys "$lang"); do
            ...
            if is_toggle_enabled "$toggle"; then
                ...
                content=$(read_instruction_file "$lang" "$file") || true
                if [[ -n "${content:-}" ]]; then
                    output_file="$lang_core_dir/$file.md"
                    {
                        echo "---"
                        echo "aiIapManaged: true"
                        echo "---"
...
generate_claude_subagents() {
    [[ ${#SELECTED_CLAUDE_CUSTOM_DEFINED_AGENTS[@]:-0} -eq 0 ]] && return 0

    local agents_dir="$OUTPUT_ROOT/.claude/agents"
    mkdir -p "$agents_dir"

    local agent_json
    for agent_json in "${SELECTED_CLAUDE_CUSTOM_DEFINED_AGENTS[@]:-}"; do
        [[ -z "$agent_json" ]] && continue
        local name desc tools model bindings_json persona_spec prompt_content
        name=$(jq -r '.name // ""' <<<"$agent_json")
        desc=$(jq -r '.description // ""' <<<"$agent_json")
        tools=$(jq -r '.tools // "Read, Glob, Grep, Write, Edit, Bash"' <<<"$agent_json")
        model=$(jq -r '.model // "sonnet"' <<<"$agent_json")
        bindings_json=$(jq -c '.ruleBindings // {}' <<<"$agent_json")
        persona_spec=$(jq -r '.personaSpecialization // "software"' <<<"$agent_json")
        [[ -z "$name" || "$name" == "null" ]] && continue
        prompt_content="You are $desc"
        prompt_content+=$'\n\n'"Follow the project rules below. They are injected from this repo's rules (languages/frameworks)."
        prompt_content+=$'\n\n---\n\n'
        prompt_content+="$(inject_rules_for_bindings "$bindings_json" "$persona_spec")"
        local output_file="$agents_dir/${name}.md"
        {
            ...
            [[ -n "$model" && "$model" != "null" ]] && echo "model: $model"
            echo "---"
            echo ""
            echo "<!-- Generated by AI Instructions and Prompts Setup -->"
            echo ""
...
generate_concatenated() {
    ...
    
    {
        ...
        if [[ -n "$preamble_file" ]]; then
            ...
            preamble_content=$(read_instruction_file "general" "$preamble_file") || true
    ...
    case "$tool" in
        ...
        claude)
            ...
            if [[ -n "$output_file" && -n "$source" ]]; then
                ...
                if [[ -n "${content:-}" ]]; then
                    echo "$content" > "$OUTPUT_ROOT/$output_file"
                    print_success "Created ${output_file}"
                fi
            fi
            # Agents are set up only via setup-agents.sh / SETUP_TYPE=agents
        :
            ;;
        github-copilot)
...
prompt_gitignore() {
    ...
    if [[ "$SCOPE" == "project" ]]; then
        ...
        print_info "Note: .ai-iap-state.json (in project) is also meant to be committed and shared."
    else
        print_info "Note: State and generated files are in your home directory ($OUTPUT_ROOT)."
    fi
