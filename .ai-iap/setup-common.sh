...
select_scope() {
    ...
    echo "  1. This project  - write to current directory (e.g. .claude/); use for version-controlled, per-repo setup"
    echo "  2. Global (user) - write to your home directory (~/.claude/); use for personal defaults across all projects"
    echo ""
...
select_setup_type() {
    ...
    echo "  1. Rules only  - languages, frameworks, structures, processes"
...
cleanup_claude_outputs() {
    cleanup_managed_claude_rules
    cleanup_managed_claude_agents
    local claude_md="$OUTPUT_ROOT/CLAUDE.md"
    if [[ -f "$claude_md" ]] && grep -q "Generated by AI Instructions and Prompts Setup" "$claude_md" 2>/dev/null; then
        rm -f "$claude_md"
    fi
}
...
save_state() {
    local langs_json docs_json fw_json structs_json procs_json subagents_json custom_agents_json
...
generate_all() {
    ...
    output_file=$(jq -r '.tool.outputFile // empty' "$WORKING_CONFIG")
    source=$(jq -r '.tool.outputFileSource // empty' "$WORKING_CONFIG")
