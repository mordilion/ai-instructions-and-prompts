...
> **Scope**: Apply these rules ONLY when working with PowerShell (`*.ps1`, `*.psm1`, `*.psd1`). These extend the general architecture guidelines.

## 1. Core Principles
- **Object-first**: Output objects, not formatted strings. Formatting is for the very edge (interactive display).
- **Composable**: Prefer functions that work well in pipelines and can be reused.
- **Explicit errors**: Treat failures as errors (exceptions) when the operation is required.
