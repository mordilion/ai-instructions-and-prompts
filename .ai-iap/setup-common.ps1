# Dot-sourced by setup-rules.ps1 and setup-agents.ps1. Do not run directly.
# Shared constants, config, state, cleanup, generation helpers.
...
# Ask where to apply config: this project or global (user profile). Sets OutputRoot, StateFile, Scope.
function Select-Scope {
    Write-Host "Where should rules and state be applied?" -ForegroundColor White
    Write-Host "  1. This project only (recommended)" -ForegroundColor White
    Write-Host "  2. Global (user profile - all projects)" -ForegroundColor White
    Write-Host ""
    $scopeChoice = Read-Host "Enter choice (1 or 2) [1]"
    if ([string]::IsNullOrWhiteSpace($scopeChoice)) { $scopeChoice = "1" }

    if ($scopeChoice -eq "2") {
        $Script:OutputRoot = [Environment]::GetFolderPath("UserProfile")
        if ([string]::IsNullOrEmpty($Script:OutputRoot)) { $Script:OutputRoot = $env:USERPROFILE }
        $Script:StateFile = Join-Path $Script:OutputRoot ".ai-iap-state.json"
        $Script:Scope = "global"
        Write-InfoMessage "Scope: global → $Script:OutputRoot"
    } else {
        $Script:OutputRoot = $Script:ProjectRoot
        $Script:StateFile = Join-Path $Script:OutputRoot ".ai-iap-state.json"
        $Script:Scope = "project"
        Write-InfoMessage "Scope: project → $Script:OutputRoot"
    }
}
...
function Select-ProjectLearningsCapture {
    param(
        [bool]$DefaultEnabled
    )

    Write-Host ""
    Write-Host "Enable project learnings capture to .ai-iap-custom/rules/general/learnings.md?" -ForegroundColor White
    Write-Host "When enabled, AIs should append stable project decisions to that file." -ForegroundColor DarkGray
    Write-Host "Note: users must re-run setup after updates so tool outputs include it." -ForegroundColor DarkGray
    Write-Host ""

    $defaultText = if ($DefaultEnabled) { "y" } else { "n" }
    $answer = Read-Host "Enable learnings capture? (y/N) [$defaultText]"
    if ([string]::IsNullOrWhiteSpace($answer)) { $answer = $defaultText }

    if ($answer -match "^[Yy]$") {
        New-ProjectLearningsFileIfMissing -EnableProjectLearnings $true
        return $true
    }
...
function Get-EnabledOptionalRules {
    param(
        [PSCustomObject]$Config,
        [string]$Lang,
        [hashtable]$ToggleStates
    )

    $langConfig = $Config.languages.$Lang
    if ($null -eq $langConfig) { return @() }
    if (-not ($langConfig.PSObject.Properties.Name -contains "optionalRules")) { return @() }

    $rules = @()
    foreach ($prop in $langConfig.optionalRules.PSObject.Properties) {
        $rule = $prop.Value
        if ($null -ne $rule.toggle -and $ToggleStates.ContainsKey($rule.toggle) -and $ToggleStates[$rule.toggle]) {
            $rules += $rule
        }
    }
...
function New-ClaudeConfig {
    ...
    $outputDir = Join-Path $Script:OutputRoot ".claude\rules"
...
"@
        ...
        $optionalRules = Get-EnabledOptionalRules -Config $Config -Lang $lang -ToggleStates $toggleStates
        foreach ($rule in $optionalRules) {
            $fileContent = Read-InstructionFile -Lang $lang -File $rule.file
            if ($null -ne $fileContent) {
                $outputFile = Join-Path $langCoreDir "$($rule.file).md"
...
function New-ProjectLearningsFileIfMissing {
    param(
        [bool]$EnableProjectLearnings
    )

    if (-not $EnableProjectLearnings) {
        return
    }

    $dir = Join-Path $Script:ProjectRoot ".ai-iap-custom\rules\general"
    $file = Join-Path $dir "learnings.md"
...
"@
    ...
    Write-SuccessMessage "Created .ai-iap-custom/rules/general/learnings.md (you can edit it anytime)"
}
...
function Save-State {
    param(
        ...
        [string]$SetupType = "both",
        [array]$SelectedCustomAgents = @()
    )
