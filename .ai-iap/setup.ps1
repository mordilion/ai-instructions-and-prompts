...
function New-ClaudeConfig {
    ...
    $claudeMdPath = Join-Path $Script:ProjectRoot "CLAUDE.md"
    $content = @"
...
"@
    ...
    foreach ($lang in $SelectedLanguages) {
        $langName = $Config.languages.$lang.name
        ...
        foreach ($file in $files) {
            ...
            if ($null -ne $fileContent) {
                $content += "`n`n## $langName - $file`n`n"
                $content += $fileContent + "`n`n---`n"
            }
        }
        ...
        if ($lang -eq "general" -and $SelectedDocumentation.Count -gt 0) {
            foreach ($docFile in $SelectedDocumentation) {
                ...
                if ($null -ne $docContent) {
                    $content += "`n`n## Documentation - $docFile`n`n"
                    $content += $docContent + "`n`n---`n"
                }
            }
        }
    }
