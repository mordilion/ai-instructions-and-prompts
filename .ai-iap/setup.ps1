...
function New-CursorConfig {
    ...
    foreach ($lang in $SelectedLanguages) {
        ...
        if ($lang -eq "general" -and $EnableProjectLearnings) {
            $content = Read-InstructionFile -Lang $lang -File "learnings"
            if ($null -ne $content) {
                $outputFile = Join-Path $langDir "learnings.mdc"
                $frontmatter = New-CursorFrontmatter -Config $Config -Lang $lang -File "learnings"
                ($frontmatter + $content) | Out-File -FilePath $outputFile -Encoding UTF8 -NoNewline
                $relativePath = $outputFile.Replace($Script:ProjectRoot, "").TrimStart("\", "/")
                Write-SuccessMessage "Created $relativePath"
            }
...
"@
        ...
        # Optional: project learnings capture (stored in .ai-iap-custom)
        if ($lang -eq "general" -and $EnableProjectLearnings) {
            $fileContent = Read-InstructionFile -Lang $lang -File "learnings"
            if ($null -ne $fileContent) {
                $outputFile = Join-Path $langCoreDir "learnings.md"
                $frontmatter = New-ClaudeFrontmatter -Paths @()
                $genHeader = @"
<!-- Generated by AI Instructions and Prompts Setup -->
<!-- https://github.com/your-repo/ai-instructions-and-prompts -->
...
"@
    ...
    foreach ($lang in $SelectedLanguages) {
        ...
        # Optional: project learnings capture (stored in .ai-iap-custom)
        if ($lang -eq "general" -and $EnableProjectLearnings) {
            $learnContent = Read-InstructionFile -Lang $lang -File "learnings"
            if ($null -ne $learnContent) {
                $content += $learnContent + "`n`n---`n`n"
            }
        }
